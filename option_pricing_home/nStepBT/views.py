...
def home(request):
    if request.method == 'POST':
        ...
        redirectURL = 'empty/' + str(value) + '/' + str(p_value) + '/' + str(n) + '/' + str(S0) + '/' + str(sigma * 100) + '/' + str(r * 100) + '/' + str(T) + '/' + str(K) + '/'
        return redirect(redirectURL)
    return render(request, 'nstep.html')


def empty(request, **kwargs):
    template = loader.get_template('Nresult.html')
    p_value, value, n, S0, sigma, r, T, K = kwargs['p_value'], kwargs['value'], kwargs['n'], kwargs['S0'], kwargs['sigma'], kwargs['r'], kwargs['T'], kwargs['K']
    # [value, n, S0, sigma, r, T, K] = kwargs.split('|')
    context = {'pVal': p_value, 'Val': value, 'n': n, 'S0': S0, 'sigma': sigma, 'r': r, 'T': T, 'K': K}
